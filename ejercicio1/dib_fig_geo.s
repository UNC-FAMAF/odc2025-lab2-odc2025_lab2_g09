...
dib_ovalo_vertical:
    ...
    */
	...
	mov x12, x4					// guarda x4 en x12
	mov x13, x5					// guarda x5 en x13
	mov x6, x0					// guarda la direccion inicial en x6
	mov x7, x0					// guarda la direccion inicial en x7

	mov x3, x4					// Y = x4
ovalo_loop_o1:
	mov x2, x4					// X = x4
	cmp x3, x10					// compara x3 con x4/2 
...
ovalo_loop_i1:
	...
	// nota: si x5 = 1, termina siendo igal a 0
	sub x3, x3, 1					// disminuye Y
	b ovalo_loop_o1					// vuelve al inicio del loopo
ovalo_loop_i2:
	stur w1,[x0]					// colorea el pixel N
	add x0, x0, 4					// Siguiente pixel
	sub x2, x2, 1					// disminuye X
    cbnz x2, ovalo_loop_i2				// Si no termino la fila, salto
	add x0, x15, x6					// salta a la siguiente linea
	mov x6, x0					// guarda la posicion actual de x0
	sub x3, x3, 1					// disminuye Y
	b ovalo_loop_o1					// vuelve al inicio del loopo
ovalo_loop_o1_end:
	mov x4, x12					// reiniciar x4
	mov x5, x13					// reiniciar x5
	mov x6, x7					// reiniciar x6
	lsr x11, x5, 1					// x11 = x5 / 2

	mul x14, x15, x4				// x14 = Y * SCREEN_WIDTH * 4
	add x0, x6, x14					// bajo X4 lineas desde la posicion inicial de x0
	mov x6, x0					// guarda la posicion actual de x0
...
ovalo_loop_o2:
	...
	cbz x11, ovalo_loop_i4				// si x5/2 = 0, salta al segundo loopi
ovalo_loop_i3:
	stur w1,[x0]					// colorea el pixel N
	add x0, x0, 4					// Siguiente pixel
	sub x2, x2, 1					// disminuye X
    cbnz x2, ovalo_loop_i3				// Si no termino la fila, salto
	lsl x14, x11, 2					// x14 = x5/2 * 4
	sub x6, x6, x14					// se mueve x5/2 a la izq. del ovalo
...
ovalo_loop_i4:
	...
	add x0, x0, 4					// Siguiente pixel
	sub x2, x2, 1					// disminuye X
    cbnz x2, ovalo_loop_i4				// Si no termino la fila, salto
